...
//상속 ( Inheritance)
//상속은 기존의 클래스를 확장하여 새로운 클래스를 이끌어 내는 것으 의미함.
//기존의 코드를 재사용함으로써 불필요한 코드 재작성을 줄여줌.
//-단일 상속만 가능(상속은 하나의 부모 클래스만 가질 수 있음)- 인터페이스 다중 상속가능함.
//- 키워드가 extends를 사용하여 상속을 표현
...
// - Object 클래스를 최상위 클래스 
//- final 클래스는 상속 불가 ...
//- abstract 클래스는 반드시 상속해서 사용해야 함. //추상클래스  메소드 정의를 하지않기때문에 재정의해야함
...
public class Person {
	 ...
	 * 
	 * -super
	 *  > super는 현재 객체의 부모객체, 한 단계 위 부모를 의미함.
	 *  > super. 
	 *  	-부모의 변수 또는 매서드를 참조
	 *  	-super.name 
	 * 		-super.go()
	 * 	>super()  // 생성자 오버로드      //오버로드 다똑같지만 매개변수가 달라야함 (type과 갯수가 달라야함)
	 * 		-부모의 생성자를 참조
	 * 		-super(10,20);
	 * 		-super("hello")
	 * 
	 ...
	 */
	// 부모인 person에 생성자 만들기...
	//반환값, 이름, 매개변수의 타입(순서)과 갯수
	public Person(int age, String name) { //Person (int , String)
		this.name =name;
		this.age =age;
	}
	//매개 변수의 타입 다른 순서로 되어 있어도 다른 생성자로 구분
