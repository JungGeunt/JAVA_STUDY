...
public class Thread10 {
	...
	public static void main(String[] args) {
		...
		// 				서로 영향을 줄 수 있는 상황이 된다 .때문에 동기화를 통해서 사용하는
		//				자원에 접근을 제한하여 작업이 종료되기 전에 접근 할 수 없게 하는 것 	
...
//	public synchronized void increaseLevel( ) { ///메서드에 동기화 설정! 
	public  void increaseLevel( ) {
		synchronized (this) {	//synchronized 블럭 설정 방법 //시작
			while(true) {		//this는 현재 클래스 SmartPhoneGame클래스가 들어 와있음
				this.level++;
				try {
					Thread.sleep(1000);
				...
				//레벨이 10의 배수가 되면 종료
				if(this.level ==5) {
					notifyAll(); //현재 대기 상태인 모든 스레드를 실행 중 상태로 전환 //우선순위가 영향을 미침 
					//notify() : 현재 대기 상태인 스레드들 중 하나만 실행 중인 상태로 전환(직접 지정 불가)
					try {
