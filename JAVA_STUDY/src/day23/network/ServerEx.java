...
public class ServerEx {
	...
	public static void main(String[] args) {
		...
		try {
			server = new ServerSocket(9999);  // port 포트 값  //backlog   //bindAddr 어느주소로 들어오면 연결해 줄지 
			
			//무한 반복( 클라이언트 접속 대기)
			while(true) {
				System.out.println("클라이언트 접속 대기 "); //accept 대기중!! 
				Socket client = server.accept(); //소켓에는 클라이언트 정보를 입력해야함 //accept로 클라이언트 정보를 받아옴
